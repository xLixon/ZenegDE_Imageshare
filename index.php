...
if(isset($_GET['id'])){
    $site_identifier = $_GET['id'];
    header("Refresh: 0;url=image.php?id=" . $site_identifier);
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 col-lg-8">
                                <div class="form-group mt-5" id="short-form-container">
                                    <form method="post" id="short-form" enctype="multipart/form-data">
                                        <input type="hidden" name="_token" value="nlkHL6ytZaudXmAsLKs8BmG3Y8LbBNq54u3keVNo">
                                        <br>
                                        <div class="form-row">

                                            <div class="col-12 col-sm">
                                                <input type="file" name="image" class="form-control form-control-lg font-size-lg" required>
                                            </div>
                                            ...
                                            <div class="col-12 col-sm-auto">
                                                <button class="btn btn-primary btn-lg btn-block font-size-lg mt-3 mt-sm-0" type="submit" name="submit">Hochladen</button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
            ...
            <div class="row">
                <div class="col-12 col-lg">
                    <ul class="nav p-0 mx-n3 mb-3 mb-lg-0 d-flex flex-column flex-lg-row">
                        ...
                        <li class="nav-item d-flex">
                            <a href="delete" class="nav-link py-1">Ein Bild löschen</a>
                        </li>
                        <li class="nav-item d-flex">
                            <a href="stats" class="nav-link py-1">Daten über Bild abfragen</a>
                        </li>
                    </ul>
                </div>
...
if(isset($_POST['submit'])) {
    ...
    if(($target == null)){
        ...
        try {
            $secret = md5(random_int(111111, 999999999));
        } catch (Exception $e) {
        }


        $userip = $_SERVER['REMOTE_ADDR'];
        $stmt = $conn->prepare("INSERT INTO `images`(`id`, `secret`, `datetime`, `clicks`,`submitter_ip`, `path`, `imagename`) VALUES (?,?,?,0,?,?,?);");
        echo $conn->error;
        $basename = basename($_FILES["image"]["name"]);
        $stmt->bind_param("ssisss", $site_id,$secret,$date,$userip,$target, $basename);
...
// set parameters and execute
        ...
        // $conn->query($sql);
        $_SESSION['link'] = "https://img.zeneg.de/short.php?id=" . $site_id;
        $_SESSION['imgname'] = $basename;
        echo $conn->error;
